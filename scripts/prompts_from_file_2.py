...
def start_btn_click():
    print(f'🚀 {state.job_count} jobs... starting now...')
...
class Script(scripts.Script):
    ...
    def ui(self, is_img2img):
            ...
            with gr.Column(scale=2, min_width=200):
                ...
                with gr.Row():
                    with gr.Column(scale=1, min_width=20):
                        start_button = gr.Button(label="Start Loop", value ='Start', elem_id=self.elem_id("start_loop_button"))
                    with gr.Column(scale=1, min_width=20):
                        stop_button = gr.Button(label="Stop Loop", value ='Abort', elem_id=self.elem_id("force_stop_button"))
            with gr.Column(scale=1, min_width=100):
        ...
        if is_img2img:
            start_button.click(fn=start_btn_click, 
                               _js="gradioApp().querySelector('.tabitem[id^=tab_]:not([style*='display: none'])').querySelector('button[id$=_generate]').click()")
        else:
            start_button.click(fn=start_btn_click, 
                               _js="gradioApp().querySelector('.tabitem[id^=tab_]:not([style*='display: none'])').querySelector('button[id$=_generate]').click()")
